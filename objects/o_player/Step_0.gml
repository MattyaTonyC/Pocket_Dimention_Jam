...
if (!instance_exists(tugging_inst)) {
	...
	var target_dis = clamp( current_dis+(mouse_dis-current_dis)/10, 50+0.75*tugging_inst.sprite_width, 175 )
	
	var current_dir = point_direction( x,y, tugging_inst.x,tugging_inst.y )
	var mouse_dir = point_direction( x,y, mouse_x,mouse_y )
	var target_dir = current_dir + angle_difference( mouse_dir, current_dir ) / 10
	...
	tugging_inst.spd.x = clamp( spd.x+lengthdir_x( tug_dis/2, tug_dir ), -1.5*movespd,1.5*movespd )
	tugging_inst.spd.y = clamp( spd.y+lengthdir_y( tug_dis/2, tug_dir ), -1.5*movespd,1.5*movespd )
